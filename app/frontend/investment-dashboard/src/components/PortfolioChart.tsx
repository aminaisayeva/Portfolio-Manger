...
export default function PortfolioChart({ history, periodDays = 90 }: PortfolioChartProps) {
  ...
  const formatTick = (d: string) => format(parseISO(d), 'MMM d');
  const formatLabel = (d: string) => format(parseISO(d), 'MMM d, yyyy');
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <AreaChart data={history} margin={{ top: 40, right: 20, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="date" {...(ticks ? { ticks } : {})} tickFormatter={formatTick} axisLine={false} tickLine={false} tick={{ fill: '#666', fontSize: 12 }} />
        <YAxis tickFormatter={v => `$${v}`} axisLine={false} tickLine={false} tick={{ fill: '#666', fontSize: 12 }} />
        ...
        <ReferenceDot x={maxPt.date} y={maxPt.value} r={5} fill="#4caf50" stroke="#fff" strokeWidth={2} isFront>
          <Label content={(props: any) => {
            ...
            return (
              <g>
                <rect x={x-textWidth/2-padding} y={y-28} width={textWidth+padding*2} height={20} fill="#fff" rx={4}/>
